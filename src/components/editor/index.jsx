...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const { focusData, lastSelectBlock, blockMousedown, containerMousedown } =
      focusUse;
    const { markline, mousedown } = useBlockDragger(
      containerRef,
      focusData,
      lastSelectBlock
    );
    ...
    return () =>
      !previewRef.value ? (
              ...
              return (
                <div
                  ...
                  onClick={
                    toolbar.handler
                      ? toolbar.handler
                      : command.commands[toolbar.commandName]
                  }
                >
                  {RenderToolbar}
                ...
                {data.value.blocks.map((block, index) => {
                  return (
                    <EditorBlock
                      ...
                      block={block}
                      onMousedown={(event) =>
                        blockMousedown(event, block, index)
                      }
                      onContextmenu={(event) =>
                        onContextMenuBlock(event, block)
                      }
        ...
        <div class="container-canvas-preview">
          ...
          <div class="footer">
            <ElButton type="primary" onClick={() => (previewRef.value = false)}>
              返回
            </ElButton>
          </div>
        </div>
